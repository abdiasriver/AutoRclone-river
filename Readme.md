# AutoRclone: rclone copy/move/sync (automaticamente) con cuentas de servicio

- [x] crear cuentas de servicio
- [x] agregar cuentas de servicio a team drive o a grupo google
- [x] Automaticamente sincronizar tema drive o copiar carpetas

[Fork of](https://github.com/xyou365/AutoRclone)
TUTORIAL ONLINE CON VIDEO
---------------------------------

**Para sistemas Linux system**: Install
...
Si estas en  Debian/Ubuntu, usa este comando
```
sudo apt-get install screen git && curl https://rclone.org/install.sh | sudo bash
```
Despues de que todas las dependencias se hayan isntalado corre este comando
```
...
Instala Rclone  [install latest rclone](https://rclone.org/downloads/). 

-  descomprime Rclone
-  Descarga este repositorio y descomprimelo dentro de la carpeta de Rclone
- Abre una consola CMD
 ```
cd ubicacioncarpetaRclone
rclone config
```

- introduce la letra "n" enter
- Coloca un nombre cualquiera para tu cuenta
- Busca el numero que pertenezca a "google drive" y ponlo , dale enter
- En client Id dale enter
- En client scope, dale enter
- Cuando te pregunte el numero de los permisos, busca el que dice "Full acces", ponlo y dale enter
- String value, dejalo igual y dale enter
- Service account, dale "y" y enter
- Use autoconfig?, dale "y" y enter
- Copia el link, abrelo en el navegador y autorizalo a tu cuenta
- Es un teamdrive?, dale "n" y enter
- Al ultimo dale "q" para quitarlo.

Corre el siguiente comando en la cmd.
```
...
`py add_to_team_drive.py -d SharedTeamDriveSrcID`
> - Add service accounts into your destination Team Drive:
`py add_to_team_drive.py -d SharedTeamDriveDstID`
...
```
py rclone_sa_magic.py -s SourceID -d DestinationID -b 1 -e 600
```
...
```
py rclone_sa_magic.py -sp YourLocalPath -d DestinationID -dp DestinationPathName -b 1 -e 600
```
